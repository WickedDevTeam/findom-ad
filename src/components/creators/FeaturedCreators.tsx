...
const FeaturedCreators = ({ creators }: FeaturedCreatorsProps) => {
  ...
  // To allow image error fallback for each creator
  return (
    <div className="relative overflow-hidden rounded-xl bg-black/30 backdrop-blur-sm border border-white/10 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {creators.slice(0, 3).map((creator) => (
          <CreatorCardWithFallback key={creator.id} creator={creator} />
        ))}
      </div>
    </div>
  );
...
// Inner component so useState is per card
const CreatorCardWithFallback = ({ creator }: { creator: Creator }) => {
