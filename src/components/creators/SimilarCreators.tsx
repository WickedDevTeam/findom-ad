...
const SimilarCreators = ({
  ...
  const similarCreators = allCreators.filter(creator => creator.id !== currentCreator.id).filter(creator => creator.categories.some(category => currentCreator.categories.includes(category))).slice(0, 2); // Show at most 2 similar creators
